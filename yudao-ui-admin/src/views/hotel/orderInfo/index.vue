    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['hotel:order-info:create']"
        >新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
                   ...
                   v-hasPermi="['hotel:order-info:export']"
        >导出
        </el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['hotel:order-info:update']"
          >修改
          </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['hotel:order-info:delete']"
          >删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="1000px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-button type="primary" @click="queryMemberInfo(form.bookingPerson)">查询</el-button>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="会员卡号" prop="memberPhone">
              <el-input v-model="form.memberPhone" disabled="true"/>
            </el-form-item>
          </el-col>

          <el-col :span="6">
            <el-form-item label="会员类型" prop="memberTypeId">
              <el-input v-model="form.memberLevelName" disabled="true"/>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="订单来源" prop="sourceId">
              <el-select v-model="form.sourceId" placeholder="请选择订单来源" clearable size="small">
                <el-option v-for="dict in this.getDictDatas(DICT_TYPE.HOTEL_ORDER_SOURCE)"
                           :key="dict.value" :label="dict.label" :value="dict.value"/>
              </el-select>
            </el-form-item>
<!--              <el-input v-model="form.sourceSubId" placeholder="请输入订单来源(小类)"/>-->
          ...
          <el-col :span="6">
            <el-form-item label="客源结构" prop="guestsSourceId">
              <el-select v-model="form.guestsSourceId" placeholder="请选择客源结构" clearable size="small">
                <el-option v-for="dict in this.getDictDatas(DICT_TYPE.HOTEL_GUEST_TYPE)"
                           :key="dict.value" :label="dict.label" :value="dict.value"/>
              </el-select>
            </el-form-item>
          </el-col>
    ...
    <member-info-dlg
      v-if="showMemInfoDlg"
      :dialog-visible="showMemInfoDlg"
      :member-name="form.bookingPerson"
      @cancel="cancelDlg('showMemInfoDlg')"
      @fillingMemInfo="fillingMemInfo"
    >
...
import MemberInfoDlg from '@/views/hotel/memberInfo/MemberInfoDlg.vue'
import TableFormNew from '@/components/Hotel/TableFormNew.vue'
...
export default {
  ...
  methods: {
     ...
     * @param memberInfo
     */
    fillingMemInfo(memberInfo) {
      this.form.memberId = memberInfo.Id
      this.form.memberPhone = memberInfo.phone
      this.form.memberName = memberInfo.name
      this.form.memberTypeId = memberInfo.levelId
      this.form.memberLevelName = memberInfo.levelName
      this.form.phone = memberInfo.phone
    },
    /** 取消按钮 */
    cancel() {
      this.open = false
    ...
    queryMemberInfo(memberId) {
      this.showMemInfoDlg = true
      this.title = '请选择预订会员'
      this.bookingPerson = memberId
    },
    /** 删除按钮操作 */
    handleDelete(row) {
      const id = row.id
