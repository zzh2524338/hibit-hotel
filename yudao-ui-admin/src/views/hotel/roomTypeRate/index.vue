  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="房型编号" prop="roomTypeId">
        <el-input v-model="queryParams.roomTypeId" placeholder="请输入房型编号" clearable
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="房价类型编号" prop="roomRateTypeId">
        <el-input v-model="queryParams.roomRateTypeId" placeholder="请输入房价类型编号" clearable
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="生效日期" prop="accDate">
        <el-date-picker v-model="queryParams.accDate" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss"
                        type="daterange"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['hotel:room-type-rate:create']">新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
                   :loading="exportLoading"
                   v-hasPermi="['hotel:room-type-rate:export']">导出
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="房价类型编号" align="center" prop="roomRateTypeId">
        <template v-slot="scope">
          <el-tag> {{ getRoomRateTypeName(scope.row.roomRateTypeId) }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['hotel:room-type-rate:update']">修改
          </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['hotel:room-type-rate:delete']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  createRoomTypeRate,
  deleteRoomTypeRate,
  exportRoomTypeRateExcel,
  getRoomTypeRate,
  getRoomTypeRatePage,
  updateRoomTypeRate
} from "@/api/hotel/roomTypeRate";
import {getRoomTypeList} from "@/api/hotel/roomType";
import {getRoomRateTypeList} from "@/api/hotel/roomRateType";
...
export default {
  ...
  methods: {
    ...
    getRoomRateTypeList() {
      ...
      getRoomRateTypeList().then(response => {
        this.roomRateTypeList = response.data;
        this.loading = false;
      })
    },
    /**
    ...
    /**
     * 根据给定的 房间类型编号 返回房间类型名称
     * @param roomRateTypeId 房间类型编号
     * @returns {*|string} 房间类型名称
     */
    getRoomRateTypeName(roomRateTypeId) {
      for (const item of this.roomRateTypeList) {
        if (item.id === roomRateTypeId) {
          return item.name;
        }
      }
      return '未知的房价类型';
